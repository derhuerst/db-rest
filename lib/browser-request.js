...
const openBrowserPage = async () => {
	if (!browserPagePromise) {
		browserPagePromise = (async () => {
			...
			if (!executablePath) {
				throw new Error('Browser transport requires CHROMIUM_EXECUTABLE_PATH to point to Chrome or Chromium.')
			}
...
const browserTransportEnabled = () => truthy.has((process.env.VENDO_BROWSER_TRANSPORT || '').toLowerCase())
